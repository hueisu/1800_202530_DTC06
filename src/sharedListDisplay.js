...
function renderSharedList(productsList, ownerID) {
  ...
  productsList.forEach((productSnapshot) => {
    ...
    $product.on("click", `#${productID}-add`, () =>
      addProductCount(productID, sharedProduct, ownerID)
    );

    // reduce
    $product.on("click", `#${productID}-reduce`, () =>
      reduceProductCount(productID, sharedProduct, ownerID)
    );

    // input
    $product.on("change", `#${productID}-count`, () =>
      editProductCount(productID, sharedProduct, ownerID)
    );
